...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const [binanceData, openInterest, fundingRate, longShortRatio] = await Promise.all([
      fetchBinanceOHLCV(symbolStr, intervalStr, 100).catch(() => []),
      fetchOpenInterest(symbolStr),
      fetchFundingRate(symbolStr),
      fetchLongShortRatio(symbolStr)
    ]);
    ...
    // Build orderflow table from last 10 candles
    const orderflowTable = binanceData.slice(-10).map((c: any) => ({
    ...
    res.json({
      ...
      footprint: binanceData.map((c: any) => ({
        time: Math.floor(c.timestamp / 1000),
        delta: c.delta,
        volume: c.volume
      })),
      orderflowTable,
      metadata: { symbol: symbolStr, interval: intervalStr, exchange: 'binance' },
      symbol: symbolStr,
