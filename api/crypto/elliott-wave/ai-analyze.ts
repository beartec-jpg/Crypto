...
  return candles
    // NOTE: The candle data now includes the high, low, and close, which Grok can use for pattern recognition.
...
  try {
    // --- 1. DESTRUCTURING (Only happens once) ---
    const {
      chartImage,
      symbol,
      timeframe,
...
    // --- FIX: Ensure chartImageData is always a string before .startsWith() is called ---
...
    // --- 3. SWING POINT DETECTION (using full historical index) ---
    const swingPoints = [];
...
    // CRITICAL FIX: Safe check for chartImageData being a real base64 string
    let messageContent: any;

    // We use .startsWith() which is safe now because chartImageData is guaranteed to be a string
...
      // --- MODEL UPGRADE: Switched to the highest quality Grok model ---
      model: 'grok-4', 
      messages: [
...
          // --- FINAL FIX: Return is time stamped (Date.now() is correct) ---
          timestamp: Date.now(),
        };
      } else {
