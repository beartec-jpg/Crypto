...
export function ConditionWeightAdjuster({ systemId, condition, onWeightChange }: Props) {
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-slate-700/40 last:border-b-0">
      <div className="flex-1 min-w-0">
        <span className="font-medium text-sm text-slate-200">{condition.name}</span>
        {condition.weight === 0 && (
      ...
      <div className="flex items-center gap-2">
        ...
        <span className={`w-14 text-right font-mono text-sm ${scoreColor}`}>
          {condition.score > 0 ? '+' : ''}
          {condition.score}
        </span>
      </div>
