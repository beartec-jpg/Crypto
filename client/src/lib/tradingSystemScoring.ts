...
export interface ScoringInput {
  ...
  /** Price history for divergence detection (last 50+ candles) */
  priceHistory?: number[];
  /** RSI value history for divergence detection (last 50+ candles) */
  rsiHistory?: number[];
  /** MACD histogram history for divergence detection (last 50+ candles) */
...
function scoreAutoFibConfluence(
  ...
  for (const fib of allFibLevels) {
    ...
    let score = ((5.0 - distToFib) / 5.0) * 50 * frozenPenalty;

    // If price is very close to the level
    if (distToFib < 0.1) {
      score = 100 * frozenPenalty;
    }
