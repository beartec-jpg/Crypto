...
export async function initFalcon(): Promise<void> {
  ...
  initPromise = (async () => {
    ...
    if (typeof window === 'undefined') {
      ...
      const wasmUrl = new URL('./falcon.wasm', import.meta.url);
...
export const falcon = {
  keygen(): { publicKey: Uint8Array; secretKey: Uint8Array } {
    ...
    try {
      const rc = m._falcon_keygen(pkPtr, skPtr);
      if (rc !== 0) throw new Error('falcon_keygen failed');
      return {
  ...
  verify(signature: Uint8Array, message: Uint8Array, publicKey: Uint8Array): boolean {
    ...
    try {
      const rc = m._falcon_verify(sigPtr, signature.length, msgPtr, message.length, pkPtr);
      return rc === 0;
    } finally {
      m._free(sigPtr);
      m._free(msgPtr);
